...
function getLemariId(item: TempatPenyimpanan) {
...
function resolveStatus(current: StorageStatus | undefined, next: StorageStatus) {
...
export function buildArsipDigitalStorageData({
  ...
  const tempatById = new Map<number, TempatPenyimpanan>();
  const lemariIdByTempatId = new Map<number, string>();
  const kantorIdByTempatId = new Map<number, string>();
