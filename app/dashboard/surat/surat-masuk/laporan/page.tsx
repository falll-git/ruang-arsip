...
import { dummySuratMasuk, type SuratMasuk } from "@/lib/data";
...
function statusBadge(status: SuratMasuk["statusDisposisi"]) {
  if (status === "Selesai")
...
function sifatBadge(sifat: SuratMasuk["sifat"]) {
...
export default function LaporanSuratMasukPage() {
  ...
  const suratScope = useMemo(() => {
    ...
    if (!viewerName) return dummySuratMasuk;

    const canViewAll =
      role === USER_ROLES.MASTER_USER || role === USER_ROLES.FULL_AKSES;
    if (canViewAll) return dummySuratMasuk;

    return dummySuratMasuk.filter((surat) =>
  ...
  const filteredSurat = useMemo(() => {
    ...
    items.sort((a, b) => {
      const aTime = parseDateString(a.tanggalTerima)?.getTime() ?? 0;
      const bTime = parseDateString(b.tanggalTerima)?.getTime() ?? 0;
      return sortBy === "terbaru" ? bTime - aTime : aTime - bTime;
    });

    return items;
  }, [filterSifat, searchTerm, sortBy, suratScope]);
  ...
  const handleExportExcel = async () => {
    await exportToExcel({
      ...
      columns: [
        ...
        { header: "Tgl Terima", key: "tanggalTerima", width: 15 },
        { header: "Sifat", key: "sifat", width: 12 },
        { header: "Status Disposisi", key: "statusDisposisi", width: 15 },
      ],
      data: filteredSurat.map((surat, idx) => ({
        ...
        tanggalTerima: formatDateDisplay(surat.tanggalTerima),
        sifat: surat.sifat,
        statusDisposisi: surat.statusDisposisi,
      })),
    });
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto">
      <FeatureHeader
        title="Laporan Surat Masuk"
        subtitle="Daftar surat masuk yang telah diterima dan didisposisikan."
        icon={<Inbox />}
        actions={
          <button
          ...
          <div className="md:col-span-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari nama surat, pengirim, atau perihal..."
              />
            </div>
          </div>
                ...
                filteredSurat.map((surat, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">
                      {formatDateDisplay(surat.tanggalTerima)}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusBadge(surat.statusDisposisi)}`}
                      >
          ...
          >
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div className="flex items-center gap-4">
                ...
                >
                  <Inbox className="w-6 h-6 text-white" aria-hidden="true" />
                </div>
                <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                        ...
                        <p className="font-semibold text-gray-800">
                          {formatDateDisplay(selectedSurat.tanggalTerima)}
                        </p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                        <label className="text-xs text-gray-500 block mb-1">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${statusBadge(selectedSurat.statusDisposisi)}`}
                        >
                          {selectedSurat.statusDisposisi}
                        </span>
                      </div>
