...
export default function DokumenLemariModal({
  ...
  }, [dokumen, filterJenis, searchTerm]);
      ...
      >
        <div className="flex items-center gap-3 border-b border-gray-100 bg-gray-50/50 px-5 py-4">
          ...
          <h3 className="truncate text-lg font-bold text-gray-900">
            {lemari.kodeLemari} {"\u00B7"} {lemari.rak} {"\u00B7"} {namaKantor}
          </h3>
        </div>
        ...
        <div className="border-b border-gray-100 bg-white px-5 py-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_180px]">
            ...
            <select
              value={filterJenis}
              onChange={(event) => setFilterJenis(event.target.value as FilterJenis)}
              className="select"
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredDokumen.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-blue-50/30">
                    ...
                    <td className="truncate px-5 py-3 text-gray-700">
                      {item.nasabah ?? "-"}
                    </td>
                    <td className="px-5 py-3">
                      <button
